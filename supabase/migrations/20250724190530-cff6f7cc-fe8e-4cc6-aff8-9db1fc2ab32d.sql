...
-- Admin users can view their own record and other admin records (for admin role)
...
-- Only admin role can insert new admin users
CREATE POLICY "Admin role can create admin users" 
ON public.admin_users 
FOR INSERT 
TO authenticated
WITH CHECK (
...
-- Admin users can update their own record, admin role can update any
...
-- Only admin role can delete admin users (except themselves)
CREATE POLICY "Admin role can delete other admin users" 
ON public.admin_users 
FOR DELETE 
TO authenticated
USING (
